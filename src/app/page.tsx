...
export default function Home() {
  const currentHost = window.location.hostname as any;
  ...
  async function authenticate() {
    ...
    try {
      const response = await fetch(
        `http://${currentHost}:3000/authentication-options`, // Use currentHost here
        {
          method: "GET",
          credentials: "include",
        }
      ...
      if (!navigator.credentials) {
        ...
        setMessage("Web Authentication API is not supported in this browser.");
        return;
      }
      ...
      const authResponse = await fetch(
        `http://${currentHost}:3000/authenticate`,
        {
          method: "POST",
